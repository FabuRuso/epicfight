	...
	protected void convertRage()
	{
		...
		else
		{
			if(!orgEntity.isAIDisabled())
			{
				...
				this.orgEntity.getDataManager().set(maninthehouse.epicfight.utils.McCompat.getEndermanScreamingParameter(), Boolean.valueOf(true));
	...
	protected void convertNormal() {
		...
		if (this.isRemote()) {
			...
			if (!orgEntity.isAIDisabled()) {
				...
				{
					this.orgEntity.getDataManager().set(maninthehouse.epicfight.utils.McCompat.getEndermanScreamingParameter(), Boolean.valueOf(false));
				}
