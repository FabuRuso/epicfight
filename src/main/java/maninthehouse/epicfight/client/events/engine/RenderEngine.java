...
public class RenderEngine {
	...
	public void buildRenderer() {
		...
		try {
			Class<?> gcPlayer = Class.forName("micdoodle8.mods.galacticraft.core.entities.player.GCEntityClientPlayerMP");
			entityRendererMap.put((Class<? extends Entity>) gcPlayer, new PlayerRenderer());
		} catch (ClassNotFoundException e) { }

		try {
			Class<?> gcPlayerMP = Class.forName("micdoodle8.mods.galacticraft.core.entities.player.GCEntityPlayerMP");
			entityRendererMap.put((Class<? extends Entity>) gcPlayerMP, new PlayerRenderer());
		} catch (ClassNotFoundException e) { }
	}
	...
	private void updateCameraInfo(int pov, double partialTicks) {
		...
		if (ClientEngine.INSTANCE.getPlayerData() == null) return;
		...
		public static void renderHand(RenderSpecificHandEvent event) {
			...
			if (ClientEngine.INSTANCE.isBattleMode()) {
				if (event.getHand() == EnumHand.MAIN_HAND
					...
					renderEngine.firstPersonRenderer.render(Minecraft.getMinecraft().player, playerdata, null, event.getPartialTicks());
				}
				event.setCanceled(true);
			}
		}
