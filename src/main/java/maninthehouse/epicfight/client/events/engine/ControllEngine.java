...
public class ControllEngine {
	...
	private void attackKeyPressed(KeyBinding key) {
		if (this.player == null || this.playerdata == null) return;
		...
		if (ClientEngine.INSTANCE.isBattleMode()) {
			...
			maninthehouse.epicfight.utils.McCompat.incrementKeyBindingPressTime(this.gameSettings.keyBindAttack);
		}

		if (this.player.getCooledAttackStrength(0) < 0.9F) {
			maninthehouse.epicfight.utils.McCompat.setKeyBindingPressTime(this.gameSettings.keyBindAttack, 0);
		}
	}
	...
	private void swapHandKeyPressed(KeyBinding key) {
		...
		if (this.playerdata.isInaction() || (cap != null && !cap.canUsedInOffhand())) {
			maninthehouse.epicfight.utils.McCompat.setKeyBindingPressTime(key, 0);
			this.setKeyBind(key, false);
		} else {
			maninthehouse.epicfight.utils.McCompat.incrementKeyBindingPressTime(key);
		}
	}
	...
	public static class Events {
		...
		public static void mouseEvent(MouseInputEvent event) {
			if (Minecraft.getMinecraft().player != null && Minecraft.getMinecraft().currentScreen == null && controllEngine != null) {
				for (Map.Entry<KeyBinding, Consumer<KeyBinding>> entry : controllEngine.keyFunctionMap.entrySet()) {
					if (maninthehouse.epicfight.utils.McCompat.getKeyBindingPressTime(entry.getKey()) > 0) {
						maninthehouse.epicfight.utils.McCompat.decrementKeyBindingPressTime(entry.getKey());
		...
		public static void moveInputEvent(InputUpdateEvent event) {
			...
			if (!controllEngine.playerCanMove(playerState)) {
				...
				maninthehouse.epicfight.utils.McCompat.setSprintToggleTimer((EntityPlayerSP)event.getEntityPlayer(), -1);
			}
		}
