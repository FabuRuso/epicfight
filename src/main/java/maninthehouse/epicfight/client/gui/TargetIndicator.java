...
public class TargetIndicator extends EntityIndicator {
    ...
    public boolean shouldDraw(EntityLivingBase entityIn) {
        ...
        if (ClientEngine.INSTANCE.getPlayerData() == null) return false;
        if (ClientEngine.INSTANCE.getPlayerData().getAttackTarget() == null) return false;
        if (Minecraft.getMinecraft().player == null) return false;
        if (Minecraft.getMinecraft().getRenderViewEntity() == null) return false;
